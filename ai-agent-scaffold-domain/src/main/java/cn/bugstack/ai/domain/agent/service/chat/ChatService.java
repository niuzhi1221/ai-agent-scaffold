...
public class ChatService implements IChatService {
    ...
    public List<AiAgentConfigTableVO.Agent> queryAiAgentConfigList() {
        ...
        ArrayList<AiAgentConfigTableVO.Agent> agentList = new ArrayList<>();
        if(null != tables){
    ...
    public String createSession(String agentId, String userId) {
        ...
        String appName = aiAgentRegisterVO.getAppName();    //  当前Agent对应的应用名称
        InMemoryRunner runner = aiAgentRegisterVO.getRunner();

        //  computeIfAbsent表示 如果userSessions中已经有这个userId，则直接返回已有的sessionId；
        //  如果没有，则使用lambda创建一个新的sessionId,并放进map中
        return userSessions.computeIfAbsent(userId, uId -> {
            Session session = runner.sessionService().createSession(appName, uId)
                    .blockingGet();     //  表示异步调用，阻塞当前线程直到session创建完成
            return session.id();
        });
    }
